...
Book::Book(const std::string& n, const std::string& c, const std::string& t, 
    ...
    //(non è specificato se il copyright debba essere attivo al momento dell'immissione)
...
void Book::setTitolo(const std::string& str){ titolo = str; }
void Book::setNomeA(const std::string& str){ nomeA = str; }
void Book::setCognomeA(const std::string& str){ cognomeA = str; }
void Book::setDataC(const Date& d){ dataC = d; }
void Book::setIsbn(const std::string& str){ isbn = str; }
...
std::ostream& operator<<(std::ostream& os, const Book& b){
        ...
        if(b.getDataC().getYear() == -4000)  os << "Nessuna data di copyright" << std::endl; 
                    //se la data non è stata cambiata si presuppone il libro non abbia data di copyright
